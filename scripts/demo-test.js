 ...
 * 
 * Note: This script loads the CommonJS build which may have WASM loading issues.
 * If you encounter WASM-related errors, the library functions correctly but
 * Node.js requires special handling for WASM modules.
 */

// Check if experimental-wasm-modules flag is set
const hasWasmFlag = process.execArgv.includes('--experimental-wasm-modules') || 
                    process.env.NODE_OPTIONS?.includes('--experimental-wasm-modules');

if (!hasWasmFlag) {
  console.error('❌ Missing required flag: --experimental-wasm-modules');
  console.error('\n💡 Please run this script using one of the following:');
  console.error('   npm run demo:test');
  console.error('   node --experimental-wasm-modules scripts/demo-test.js');
  console.error('\n   Or set NODE_OPTIONS:');
  console.error('   NODE_OPTIONS="--experimental-wasm-modules" node scripts/demo-test.js');
  process.exit(1);
}

// Use dynamic import for ESM build (works better with WASM)
let generateKeys, deriveKeysFromMnemonic, signPsbt;

async function loadModule() {
  try {
    // Load ESM module using dynamic import (works with WASM)
    const module = await import('../dist/index.mjs');
    generateKeys = module.generateKeys;
    deriveKeysFromMnemonic = module.deriveKeysFromMnemonic;
    signPsbt = module.signPsbt;
    return true;
  } catch (error) {
    console.error('❌ Failed to load module:', error.message);
    if (error.message && error.message.includes('.wasm')) {
      console.error('\n💡 WASM loading error detected.');
      console.error('   Make sure you are running with: --experimental-wasm-modules flag');
      console.error('   Run: npm run demo:test');
    } else {
