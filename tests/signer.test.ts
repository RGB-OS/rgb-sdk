...
describe('signer', () => {
  ...
  describe('signPsbt', () => {
    ...
    it('should work with different network formats', async () => {
      const signed1 = await signPsbt(testMnemonic, utxoUnsignedPsbt, 'testnet');
      const signed2 = await signPsbt(testMnemonic, utxoUnsignedPsbt, 'testnet'); // Both as string
    ...
    it('should be idempotent (signing already signed PSBT)', async () => {
      ...
      const firstSigned = await signPsbt(testMnemonic, utxoUnsignedPsbt, 'testnet');
  ...
  describe('validation', () => {
    it('should throw ValidationError for empty mnemonic', async () => {
      await expect(signPsbt('', utxoUnsignedPsbt, 'testnet')).rejects.toThrow(ValidationError);
      await expect(signPsbt('', utxoUnsignedPsbt, 'testnet')).rejects.toThrow('mnemonic');
    });

    it('should throw ValidationError for invalid mnemonic', async () => {
      await expect(signPsbt('invalid mnemonic', utxoUnsignedPsbt, 'testnet')).rejects.toThrow(ValidationError);
    });

    it('should throw ValidationError for empty PSBT', async () => {
      await expect(signPsbt(testMnemonic, '', 'testnet')).rejects.toThrow(ValidationError);
      await expect(signPsbt(testMnemonic, '', 'testnet')).rejects.toThrow('psbtBase64');
    });

    it('should throw ValidationError for invalid PSBT format', async () => {
      await expect(signPsbt(testMnemonic, 'invalid-base64', 'testnet')).rejects.toThrow(ValidationError);
      await expect(signPsbt(testMnemonic, 'not-a-psbt', 'testnet')).rejects.toThrow(ValidationError);
    });

    it('should throw ValidationError for invalid base64', async () => {
      await expect(signPsbt(testMnemonic, '!!!invalid!!!', 'testnet')).rejects.toThrow(ValidationError);
    });
  });

  describe('error handling', () => {
    it('should throw CryptoError for cryptographic failures', async () => {
      // This would typically require mocking internal dependencies
      // For now, we verify that valid inputs don't throw
      await expect(signPsbt(testMnemonic, utxoUnsignedPsbt, 'testnet')).resolves.toBeTruthy();
    });
  });
